...
class LazyFrameworkGUI(QMainWindow):
    ...
    def open_module_in_tab(self, module_path: str):
          ...
          if module_path not in self.framework.modules:
               ...
               return
          try:
               mod_instance = self.framework.load_module(module_path)
          except Exception as e:
               self.append_output(f"[red]Gagal load module: {e}[/]")
               return
          for i in range(self.tabs.count()):
               widget = self.tabs.widget(i)
               if isinstance(widget, ModuleTab) and widget.module_name == module_path:
                    self.tabs.setCurrentIndex(i)    # Fokus ke tab yang sudah ada
                    return
          tab = ModuleTab(
               framework=self.framework,
               module_name=module_path,
               module_instance=mod_instance,
               parent=self.tabs          # parent = QTabWidget agar _request_close bisa findIndex
          )
          self._populate_module_tab_options(tab, mod_instance)

          # Tab label = nama akhir path, misal "recon/nmap" → "nmap"
          short_name = module_path.split("/")[-1]
          idx = self.tabs.addTab(tab, f"⚡ {short_name}")
          self.tabs.setCurrentIndex(idx)

          # Tambah tombol close (X) di tab header
          self.tabs.setTabsClosable(True)
          self.tabs.tabCloseRequested.connect(self._close_module_tab)

          self.append_output(f"[green]✓ Opened module tab: {module_path}[/]")
    ...
    def _populate_module_tab_options(self, tab: "ModuleTab", mod_instance):
          ...
          for key, meta in options.items():
               label = QLabel(key)
               field = QLineEdit()
               field.setText(str(meta.get('value', '') or meta.get('default', '')))
               field.setPlaceholderText(meta.get('description', ''))
               # Saat nilai berubah, update langsung ke module options
               field.textChanged.connect(
                     lambda val, k=key, m=mod_instance: m.options.__setitem__(k, {'value': val})
               )
    ...
    def _close_module_tab(self, index: int):
          ...
          if isinstance(widget, ModuleTab):
               if widget.module_runner and widget.module_runner.isRunning():
    ...
    _CAT_META = {
        "recon":    {"color": "#50fa7b", "icon": "📡", "label": "Recon"},
        "strike":   {"color": "#ff5555", "icon": "⚡", "label": "Strike"},
        "hold":     {"color": "#f1fa8c", "icon": "🔒", "label": "Hold"},
        "ops":      {"color": "#ffb86c", "icon": "🛠️",  "label": "Ops"},
        ...
        for cat in CAT_ORDER:
            ...
            for sub_key in sorted(tree_data[cat].keys()):
                ...
                if sub_key:
                    ...
                    sub_item.setForeground(0, QColor(color + "cc"))
    ...
    def load_selected_module(self, item):
        """Load module yang dipilih dari QTreeWidget (double-click)."""
        if not item:
            return

        module_path = item.data(0, Qt.ItemDataRole.UserRole)
        if not module_path:
            # Klik pada folder → toggle expand/collapse
    ...
    def on_module_finished(self):
        ...
        if self.module_runner:
            ...
            self.module_runner.wait(1500)   # Tunggu max 1.5 detik
            self.module_runner = None
